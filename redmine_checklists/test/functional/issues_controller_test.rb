...
class IssuesControllerTest < ActionController::TestCase
  ...
  def test_show_index_with_checklists_filter
    get :index, :project_id => 1, :set_filter => '1', :f => ['checklists_status', ''], :op => { :checklists_status => '=' }, :v => { :checklists_status => ['1'] }
    assert_response :success
    assert_equal assigns(:issues).map(&:id), [2, 1]

    get :index, :project_id => 1, :set_filter => '1', :f => ['checklists_status', ''], :op => { :checklists_status => '=' }, :v => { :checklists_status => ['0'] }
    assert_response :success
    assert_equal assigns(:issues).map(&:id), [1]

    get :index, :project_id => 1, :set_filter => '1', :f => ['checklists_item', ''], :op => { :checklists_item => '~' }, :v => { :checklists_item => ['todo'] }
    assert_response :success
    assert_equal assigns(:issues).map(&:id), [2, 1]

    get :index, :project_id => 1, :set_filter => '1', :f => ['checklists_item', ''], :op => { :checklists_item => '~' }, :v => { :checklists_item => ['Third'] }
    assert_response :success
    assert_equal assigns(:issues).map(&:id), [2]

    get :index, :project_id => 1, :set_filter => '1', :f => ['checklists_item', ''], :op => { :checklists_item => '!*' }
    assert_response :success
    assert_equal assigns(:issues).map(&:id).include?(1), false
    assert_equal assigns(:issues).map(&:id).include?(2), false
  end
  ...
  def test_save_order_position
    ...
    issue = Issue.find(1)
    @request.session[:user_id] = 1
    xhr :put, :update, :issue => parameters, :project_id => issue.project, :id => issue.to_param
