...
class DealsControllerTest < ActionController::TestCase
  ...
  def test_filter_by_ids
    @request.session[:user_id] = 1
    ids = [3,2]
    get :index, :project_id => 2, :set_filter => 1, "f"=>["ids", ""], "op"=>{"ids"=>"="}, "v"=>{"ids"=>[ids.join(",")]}
    assert_response :success
    assert_equal ids.sort, assigns(:deals).map(&:id).sort
  ...
  def test_filter_by_ids_any
    ...
    assert_response :success
    assert_equal Project.find(1).deals.map(&:id).sort, assigns(:deals).map(&:id).sort
  end if Redmine::VERSION.to_s >= '3.3'

  def test_filter_by_ids_more_than
    @request.session[:user_id] = 1
    get :index, :project_id => 2, :set_filter => 1, "f"=>["ids", ""], "op"=>{"ids"=>">="}, "v"=>{"ids"=>[3]}
    assert_response :success
    assert_equal [3, 4, 5], assigns(:deals).map(&:id).sort
  end if Redmine::VERSION.to_s >= '3.3'
  ...
  def test_put_update
    ...
    put :update, :id => 3, :deal => {:name => new_name, :currency => "GBP", :price => 23000}
  ...
  def test_get_index_with_product_filter
    @request.session[:user_id] = 1
    get :index, :set_filter => '1', :f => ['products', ''], :op => { 'products' => '*' }
  ...
  def test_get_index_with_product_category_filter
    @request.session[:user_id] = 1
    get :index, :set_filter => '1', :f => ['product_category_id', ''], :op => { 'product_category_id' => '=' }, :v => { :product_category_id => ['1'] }
    assert_equal 1, assigns(:deals).count
    assert_select 'table.deals.index h1.deal_name a', /First deal with contacts/
  end if RedmineContacts.products_plugin_installed?
end
