...
class ContactsControllerTest < ActionController::TestCase
  ...
  def test_get_index_with_all_fields
    @request.session[:user_id] = 1
    get :index, crm_query_params.merge({:c => ContactQuery.available_columns.map(&:name), :contacts_list_style => "list"})
    assert_response :success
    assert_template :index
  ...
  def test_filter_by_ids_equal
    ...
    assert_response :success
    assert_equal ids.sort, assigns(:contacts).map(&:id).sort
  end if Redmine::VERSION.to_s >= '3.3'

  def test_filter_by_ids_any
    @request.session[:user_id] = 1
    get :index, :project_id => 1, :set_filter => 1, "f"=>["ids", ""], "op"=>{"ids"=>"*"}
    assert_response :success
    assert_equal Project.find(1).contacts.map(&:id).sort, assigns(:contacts).map(&:id).sort
  end if Redmine::VERSION.to_s >= '3.3'

  def test_filter_by_ids_more_than
    @request.session[:user_id] = 1
    get :index, :project_id => 1, :set_filter => 1, "f"=>["ids", ""], "op"=>{"ids"=>">="}, "v"=>{"ids"=>[3]}
    assert_response :success
    assert_equal [3, 4, 5], assigns(:contacts).map(&:id).sort
  end if Redmine::VERSION.to_s >= '3.3'

  def test_filter_by_ids_less_than
    @request.session[:user_id] = 1
    get :index, :project_id => 1, :set_filter => 1, "f"=>["ids", ""], "op"=>{"ids"=>"<="}, "v"=>{"ids"=>[2]}
    assert_response :success
    assert_equal [1,2], assigns(:contacts).map(&:id).sort
  end if Redmine::VERSION.to_s >= '3.3'

  def test_filter_by_ids_between
    @request.session[:user_id] = 1
    get :index, :project_id => 1, :set_filter => 1, "f"=>["ids", ""], "op"=>{"ids"=>"><"}, "v"=>{"ids"=>["1", "3"]}
    assert_response :success
    assert_equal [1,2,3], assigns(:contacts).map(&:id).sort
  end if Redmine::VERSION.to_s >= '3.3'
