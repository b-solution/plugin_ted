...
class Deal < ActiveRecord::Base
  ...
  if ActiveRecord::VERSION::MAJOR >= 4
    has_and_belongs_to_many :related_contacts, lambda{ order("#{Contact.table_name}.last_name, #{Contact.table_name}.first_name").uniq }, :class_name => 'Contact'
  else
  ...
  def expected_revenue
    self.probability ? (self.probability.to_f / 100) * self.price.to_f : self.price
  end
  ...
  def copy_from(arg)
    ...
    if Redmine::Plugin.load && Redmine::Plugin.installed?(:redmine_products) && Redmine::Plugin.find(:redmine_products).version >= '2.0.2'
      deal.lines.each do |line|
        self.lines.build(line.attributes)
      end
    end
    self
  end
