    ...
    module IssueQueryPatch
      def self.included(base)
        ...
        base.class_eval do
          ...
          alias_method_chain :initialize_available_filters, :contacts
      ...
      module InstanceMethods
        ...
        def initialize_available_filters_with_contacts
          ...
          selected_contacts = filters['contacts'].present? ? Contact.visible.where(:id => filters['contacts'][:values]).map { |c| [c.name, c.id.to_s] } : []
          selected_companies = filters['companies'].present? ? Contact.visible.where(:id => filters['companies'][:values]).map { |c| [c.name, c.id.to_s] } : []
          add_available_filter('contacts', :type => :list_optional, :field_format => 'contact', :name => l(:field_contacts),
                                           :values => selected_contacts) if !available_filters.key?('contacts') && User.current.allowed_to?(:view_contacts, project, :global => true)

          add_available_filter('companies', :type => :list, :field_format => 'company', :name => l(:field_companies),
                                            :values => selected_companies) if !available_filters.key?('companies') && User.current.allowed_to?(:view_contacts, project, :global => true)

          add_available_filter('deal', :type => :integer, :name => l(:label_deal)) if !available_filters.key?('deal') && User.current.allowed_to?(:view_deals, project, :global => true)
        end

        def contact_query_values(values)
          contacts_for_select(nil, :where => {:id => values}, :short_label => true)
        end
      end
    end
  end
