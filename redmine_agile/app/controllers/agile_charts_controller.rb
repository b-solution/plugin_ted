...
private
  ...
  def render_data(options={})
    case @chart
    when "lead_time"
      data = RedmineAgile::LeadTimeChart.render(@issues, options)
    when "average_lead_time"
      data = RedmineAgile::LeadTimeChart.render(@issues, options.merge(:average_lead_time => true))
    when "burnup"
      data = RedmineAgile::BurnupChart.render(@issues, options)
    when "trackers_cumulative_flow"
      data = RedmineAgile::TrackersCumulativeFlowChart.render(@issues, options)
    when "cumulative_flow"
      data = RedmineAgile::CumulativeFlowChart.render(@issues, options)
    when "issues_velocity"
      data = RedmineAgile::VelocityChart.render(@issues, options)
    when "work_burnup_hours"
      data = RedmineAgile::WorkBurnupChart.render(@issues, options.merge(:estimated_unit => 'hours'))
    when "work_burnup_sp"
      data = RedmineAgile::WorkBurnupChart.render(@issues, options.merge(:estimated_unit => 'story_points'))
